...
阅读项目代码，找出bug和仅仅是占位链路未完整实现的链路以及存在的问题，修复bug和问题以及解决存在的问题，更新docker容器中的代码。根据8. 全链路Bug 排查给出优化建议驱动下一次任务。循环直到功能全部实现,bug和问题仅有p2级
...
部署前：`docker stats --no-stream` + `docker system df`。已知泄漏：`InMemoryIdempotencyService` 无 TTL、SseEmitter 僵尸、Sandbox 无清理。
