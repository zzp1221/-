...
当前 `master` 的目标不是替换 Java 入口、Redis Streams、SSE 协议或下载签名机制，而是在 Python Agent 内部把 `RESOURCE_GENERATION` 从“按 `resourceType` 选一个生成器”升级为“LangGraph 并发 fan-out 的资源包工作流”。
...
- LangChain multi-agent 文档建议在需要上下文隔离、并行化、专门职责和顺序约束时使用多智能体模式；当前实现采用 Router/Custom workflow 思路。
- LangGraph 文档定位为低层级、有状态、长任务 Agent 编排运行时；当前生产 `stream()` 已通过 `StateGraph.astream(..., stream_mode=["updates", "custom"])` 执行资源包图。
- Deep Agents 文档强调 coordinator-worker、规划、子 Agent 隔离和文件/记忆能力；当前实现没有引入 `deepagents` 依赖，而是吸收其 coordinator-worker 思路，保留当前项目已有 Agent 和 SSE 体系。
...
5. Graph 中资源 Agent 按用户选择的 `resourceTypes[]` 并发 fan-out；前端只暴露原有资源生成类型，选择 1 个或多个都可以，后端不再自动补齐到 5 类。未传 `resourceTypes/resourceType` 时仅沿用旧默认讲解文档 `DOCUMENT`。
6. `RESOURCE_GENERATION` 下的 `QueryRewriteAgent` 优先使用 LLM 改写；LLM 429/异常或结构校验失败时允许退回本地 direct rewrite，以保证单资源生成不会在改写阶段直接失败。
...
- `RESOURCE_GENERATION` 已符合 Graph 多智能体资源包目标：入口是 LangGraph `StateGraph`，生产流式路径使用 `astream` 运行图，先串行完成 query rewrite 与 retrieval，再按用户请求的资源类型显式并发 fan-out 到对应资源 Agent 节点，最后由 `bundle_synthesizer` 汇总。并发测试会让多个被请求的 Agent 在同一个 barrier 前等待，只有真实并发启动才会通过。
- 各个路由 Agent 已做可路由性校验：`supervisor_routes.json` 中的 Agent 必须存在于 `agent_registry`，或显式属于虚拟节点 `resource_bundle`。这保证“配置上能调到”的 Agent 不会是悬空名字。旧 `{generation_agent}` 单生成器兼容占位已从 active 代码和测试中清理。
...
当前仍由 `PythonAgentSupervisor.resolve_route()` 承担。它决定任务进入资源包、视频、辅导、评估、路径规划等路径。当前 `master` 中，`RESOURCE_GENERATION` 不再按单个资源类型选择生成 Agent，而是进入 `resource_bundle`。
...
负责 LLM 查询改写和关键词提取。Graph 中它必须先于检索执行；`RESOURCE_GENERATION` 下 LLM 改写失败时允许退回本地 direct rewrite，避免 429/临时异常把单资源任务直接打失败。
