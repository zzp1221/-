...
# mediapipe API compatibility: 0.10.0-0.10.9 uses solutions, 0.10.30+ uses tasks
try:
...
if mp_face_mesh is None:
    raise EnvironmentError(
        "mediapipe>=0.10,<0.10.10 required for avatar preprocessing. "
        "On this Python version, run preprocessing inside Docker: "
        "docker compose run --rm python-agent python scripts/preprocess_avatar.py --input ..."
    )
