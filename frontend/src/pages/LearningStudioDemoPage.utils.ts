...
import { ConversationMessageItem } from '../api/conversation';
import { ChatMessage, EngineState, EngineTaskSnapshot, QnaState } from './LearningStudioDemoPage.types';
...
export function mapConversationHistory(history: ConversationMessageItem[]): ChatMessage[] {
  ...
  return messages.length > 0
    ? messages
    : [{ id: 'qna-greeting', role: 'assistant', content: '你好，我是智学引擎，评估和推荐你的个性化学习方案。请问有什么可以帮你？' }];
}
...
export function isTaskTerminal(snapshot: EngineTaskSnapshot): boolean {
...
export function normalizeRestoredQnaMessages(snapshot: PersistedQnaSnapshot): ChatMessage[] {
  if (!Array.isArray(snapshot.qnaMessages) || snapshot.qnaMessages.length === 0) {
    return [{ id: 'qna-greeting', role: 'assistant', content: '你好，我是智学引擎，评估和推荐你的个性化学习方案。请问有什么可以帮你？' }];
  }
